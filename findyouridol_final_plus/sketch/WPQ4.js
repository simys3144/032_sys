function WPQ4()
{
  ...
  this.setup = function()
  {
    pQ = this.sceneManager.wpQ;
    nextButton = new Clickable();
    array0 = new Array(1);
    array1 = new Array(1);
    question = new Question(pQ);
    createCanvas(600, 600);
    personality.resize(600, 600);
  ...
  this.draw = function() {
    image(this.sceneManager.personality1, width/2, height/2, 600, 600);
    question.setQuestion();
    question.display(questionNum);
    setButton(questionNum, pQ[questionNum*2+1].length);
    ...
    fill(220);
    ellipse(width/3 + i*20*5, height/12, 10, 10);
  ...
  this.mousePressed = function() { //씬매니저 성격 상 this.와 같이 써야 해서 이렇게밖에...ㅠㅠ
    ...
    if (mouseX > x1 && mouseX < x2 && mouseY > y1 && mouseY < y2) {
      if (b0 == false && b1 == false) {
        ...
        this.sceneManager.showScene(Loading1);
      }
    }
  }
