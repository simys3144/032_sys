...
this.draw = function() {
  ...
  setButton(questionNum, vQ[questionNum*2+1].length);
  conclusion = offButton_2(array0, array1, array2);
  print(conclusion);
  next();
  ...
  //선택에 따라 바뀌는 캐릭터
  if (b0 == false && b1 == false && b2 == false) image(this.sceneManager.genie2, width/5, height*3/4);
...
function offButton_2(array0, array1, array2) {
  ...
  buttons[0].onPress = function() { //버튼 0이 눌리면
    ...
    buttons[2].color = color(255); //다른 버튼은 흰색으로
  ...
  con1 = concat(array0, array1);
  con = concat(con1, array2)
  // print(con);
  return con;
}
...
this.mousePressed = function() { //씬매니저 성격 상 this.와 같이 써야 해서 이렇게밖에...ㅠㅠ
  x = mouseX;
  y = mouseY;
  ...
  if (mouseX > x1 && mouseX < x2 && mouseY > y1 && mouseY < y2) {
    if (b0 == false && b1 == false && b2 == false) {
      ...
      this.sceneManager.showScene(VQ5);
    }
  }
}
